...
type TabType = "catalog" | "twin" | "pipeline" | "ingest" | "delivery" | "batch" | "benchmark";
...
export default function Home() {
  ...
  const [exportData, setExportData] = useState<ExportJsonResult | null>(null);
  ...
  const fetchCatalogSummary = useCallback(async () => {
    try {
      const data = await api.getProductSummary();
      setSummary(data);
    } catch (err: unknown) {
  ...
  const fetchProducts = useCallback(async (selectId?: string) => {
    try {
      ...
      setError(err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Failed to fetch products"));
    } finally {
      setLoading(false);
    }
  ...
  const fetchJobStatus = useCallback(async (productId: string) => {
    try {
      const data = await api.getJobStatus(productId);
      setPipelineJob(data);
    } catch (err: unknown) {
    ...
    (async () => {
      try {
        ...
        if (data.length > 0) {
          ...
          const dData = await api.getProduct(firstId);
          if (active) setCurrentProduct(dData);
  ...
  const handleLaunchDemoMode = async () => {
    ...
    try {
      const data = await api.runDemoPipeline();
      ...
      alert(`Demo Error: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Failed to run demo")}`);
    } finally {
  ...
  const handleSeedDemoProducts = async () => {
    ...
    try {
      ...
      alert(`Demo Seed Error: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Failed to seed demo products")}`);
    } finally {
  ...
  const handleReviewAction = async (
    ...
    try {
      ...
      await api.submitReview(currentProduct.id, payload);
      ...
      alert(`Review Error: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Unknown error occurred")}`);
    } finally {
  ...
  const handleApproveProduct = async () => {
    if (!currentProduct) return;
    try {
      await api.approveProduct(currentProduct.id);
      await fetchProductDetails(currentProduct.id);
      await fetchProducts(currentProduct.id);
      alert("Product Twin successfully approved and published.");
    } catch (err: unknown) {
      alert(`Approval Failed: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Unknown error occurred")}`);
    }
  };
  ...
  const handleExportJSON = async () => {
    if (!currentProduct) return;
    try {
      const data = await api.exportProductJson(currentProduct.id);
      setExportData(data);
      setExportModalOpen(true);
      setCopiedExport(false);
    } catch (err: unknown) {
      alert(`Export Failed: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Unknown error occurred")}`);
    }
  };
  ...
  const handleExportCSV = () => {
    if (!currentProduct) return;
    window.open(api.getExportCsvUrl(currentProduct.id), "_blank");
  };
  ...
  const handleIngestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await api.ingestProduct(
        partNumber.trim(),
        brand.trim(),
        description.trim(),
        file
      ...
      alert(`Ingestion Error: ${err instanceof api.ApiError ? err.detail : (err instanceof Error ? err.message : "Unknown error occurred")}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--background)" }}>
          ...
          <nav style={{ display: "flex", gap: "0.25rem", background: "#f1f5f9", padding: "0.2rem", borderRadius: "6px" }}>
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === "delivery" ? "var(--navy)" : "var(--text-secondary)",
                boxShadow: activeTab === "delivery" ? "var(--shadow-sm)" : "none",
              }}
            >
              Delivery Center
            </button>
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === "batch" ? "var(--navy)" : "var(--text-secondary)",
                boxShadow: activeTab === "batch" ? "var(--shadow-sm)" : "none",
              }}
            >
              Batch Jobs
            </button>
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === "benchmark" ? "var(--navy)" : "var(--text-secondary)",
                boxShadow: activeTab === "benchmark" ? "var(--shadow-sm)" : "none",
              }}
            >
              Benchmark
            </button>
      ...
      <main style={{ maxWidth: "1360px", margin: "0 auto", padding: "1.75rem 2rem 4rem" }}>
        ...
        {/* ─── TAB 5: DELIVERY CENTER ────────────────────────────────────── */}
        {activeTab === "delivery" && <DeliveryCenter />}

        {/* ─── TAB 6: BATCH JOBS ─────────────────────────────────────────── */}
        {activeTab === "batch" && <BatchJobs />}
